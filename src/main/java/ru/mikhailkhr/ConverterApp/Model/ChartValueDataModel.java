...
public class ChartValueDataModel {
	...
	public List<Map<String, Object>> getChartData(LocalDate dateFrom, LocalDate dateTo, int numCodeFrom, int numCodeTo,
			Unit unit) {
		List<ValutePair> valuteValues = valuteValuesJdbcDao.getFromToValueInformation(dateFrom, dateTo, numCodeFrom,
				numCodeTo);
		...
		for (ValutePair pair : valuteValues) {
			...
			if (compareDate.equals(chartEntryLocalDate)) {
				...
				ZonedDateTime zonedDataForChartEntry = LocalDateTime.of(chartEntryLocalDate, LocalTime.MIDNIGHT)
	...
	private List<LocalDate> getValuteValueMissingDates(List<ValutePair> valuteValues, LocalDate dateFrom,
			LocalDate dateTo, Unit unit) {
		...
		while (!dateTo.isBefore(compareDate)) {
			if (valutePairIndex < valuteValues.size()) {
				...
				System.out.println("valuteValueDate = " + valuteValueDate);
	...
	private List<LocalDate> getMissingDates(List<ValutePair> valuteValues, LocalDate dateFrom, LocalDate dateTo,
			Unit unit) {
		int valutePairIndex = 0;
		List<LocalDate> missingDates = new ArrayList<LocalDate>();
		// Find all valute value dates that missing in the database
		for (LocalDate compareDate = dateFrom.plusDays(0); !compareDate
				.isAfter(dateTo); compareDate = ValuteAppDateUtils.datePlusUnit(compareDate, unit)) {
