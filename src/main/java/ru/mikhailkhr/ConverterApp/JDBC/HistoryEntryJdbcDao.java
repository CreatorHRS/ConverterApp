...
public class HistoryEntryJdbcDao
{
	...
	private final String INSERT_HISTORY_ENTRY = "INSERT INTO converterAppSchema.historyEntries (from_valute_value_id, to_valute_value_id, number, user_id, date_of_convertion, time_of_convertion) " + "VALUES (?, ?, ?, ?, ?, ?)";
	...
	public List<HistoryEntry> selectHistoryByUserId(int userId)
	{
		...
		RowMapper<HistoryEntry> selectHistoryRowMapper = new RowMapper<HistoryEntry>()
		{
			...
			public HistoryEntry mapRow(ResultSet rs, int rowNum) throws SQLException
			{
				...
				/*
				 * Using all needed information from sql result calculate all values
				 */
	...
	{
		// String.format(SELECT_HISTOTY_BY_USER_ID_AND_BY_DATE, userId,
		// date.format(postresFormatter)),
